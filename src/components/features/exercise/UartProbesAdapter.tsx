...
const UartProbesAdapter = ({ onPositionChange, bounds, readOnly = false }: UartProbesAdapterProps) => {
  const {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (!isDragging || !adapterRef.current) return;
      const container = adapterRef.current.parentElement;
      if (!container) return;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isConnected ? 'bg-gray-700/50' : readOnly ? 'bg-gray-700/30' : 'bg-gray-700/30 hover:bg-gray-600/30'
              )}
            >
              <div
