...
const PCBViewer = () => {
  ...
  useLayoutEffect(() => {
    ...
    const updateDimensionsAndBounds = () => {
      ...
      setBounds(pcbElement.getBoundingClientRect());
    };
  ...
  const getWireOrigin = (probeNumber: 'first' | 'second') => {
    if (!multimeterPosition || !pcbContainerRef.current) return null;
    const pcbRect = pcbContainerRef.current.getBoundingClientRect();
    const relativeX = multimeterPosition.x - pcbRect.left;
    const relativeY = multimeterPosition.y - pcbRect.top;
    const offsetX = probeNumber === 'first' ? 30 : 60;
    const offsetY = 120;
